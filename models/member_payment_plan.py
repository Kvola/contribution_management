...
class MemberPaymentPlan(models.Model):
    ...
    amount = fields.Monetary(
        string="Montant total", currency_field="currency_id", required=True, default=0.0
    )
    ...
    fee_amount = fields.Monetary(
        string="Montant des frais", currency_field="currency_id"
    )
    ...
    paid_installments = fields.Integer(
        string="Échéances payées", compute="_compute_stats", store=True
    )
    remaining_installments = fields.Integer(
        string="Échéances restantes", compute="_compute_stats", store=True
    )
    ...
    completion_rate = fields.Float(
        string="Taux de completion", compute="_compute_stats", store=True
    )

    installment_ids = fields.One2many(
        "member.payment.installment", "payment_plan_id", string="Échéances", copy=False
    )
    ...
    auto_allocate_to_cotisations = fields.Boolean(
        string="Allocation automatique aux cotisations",
        default=True,
        help="Les paiements d'échéances seront automatiquement alloués aux cotisations",
    )
    ...
    allocation_method = fields.Selection(
        ...
        default="auto",
        help="Méthode de répartition par défaut pour toutes les échéances de ce plan",
    )

    @api.model
    def create(self, vals):
        """Override create pour configurer les échéances créées"""
        plan = super().create(vals)

        # Configurer les échéances si allocation automatique activée
        if plan.auto_allocate_to_cotisations:
            plan._configure_installments_allocation()

        return plan

    def _configure_installments_allocation(self):
        """Configure l'allocation automatique pour toutes les échéances du plan"""
        for installment in self.installment_ids:
            installment.write(
                {
                    "auto_allocate": self.auto_allocate_to_cotisations,
                    "allocation_method": self.allocation_method,
                }
            )

            # Si pas encore configuré, faire la sélection automatique
            if not installment.cotisation_ids:
        ...
        for plan in self:
            if plan.start_date and plan.frequency and plan.number_of_installments:
                if plan.frequency == "weekly":
                    plan.end_date = plan.start_date + timedelta(
                        weeks=plan.number_of_installments - 1
                    )
                elif plan.frequency == "biweekly":
                    plan.end_date = plan.start_date + timedelta(
                        weeks=(plan.number_of_installments - 1) * 2
                    )
                else:  # monthly
                    plan.end_date = plan.start_date + relativedelta(
                        months=plan.number_of_installments - 1
                    )
            else:
                plan.end_date = False
    ...
    def action_complete(self):
        ...
        if all(i.state == "paid" for i in self.installment_ids):
            ...
            raise UserError(
                "Toutes les échéances doivent être payées pour terminer le plan."
            )
    ...
    def generate_installments(self):
        ...
        if (
            not self.total_amount
            or not self.number_of_installments
            or not self.start_date
        ):
            raise UserError(
                "Tous les champs requis doivent être renseignés pour générer les échéances."
            )

        # Supprimer les échéances existantes si elles ne sont pas payées
        existing_installments = self.installment_ids.filtered(
    ...
    @api.depends(
        "installment_ids", "installment_ids.state", "installment_ids.amount_paid"
    )
    def _compute_stats(self):
        """Calcule les statistiques du plan"""
        for plan in self:
    ...
    def create(self, vals):
        ...
        if not vals.get("name"):
            vals["name"] = (
                self.env["ir.sequence"].next_by_code("member.payment.plan") or "/"
            )
    ...
    def _cron_send_payment_reminders(self):
        ...
        plans_with_reminders = self.search(
            [("state", "=", "in_progress"), ("auto_reminder", "=", True)]
        )
    ...
    def _send_reminder_email(self, plan, installments):
        ...
        try:
            template = self.env.ref(
                "contribution_management.email_template_payment_reminder", False
            )
            if template:
